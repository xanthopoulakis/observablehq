...
export async function createReferendum2015Article({ Inputs, html, md }) {
  ...
  body.append(
    markdown_block(
      ...
      **υψηλότερο ποσοστό σε οποιαδήποτε πανελλαδική εκλογική αναμέτρηση από το 1996 έως το 2024** στα δεδομένα
      του Υπουργείου Εσωτερικών, σχεδόν διπλάσιο από τη δεύτερη υψηλότερη τιμή (4,46% στις ευρωεκλογές του 2019) και έως
  ...
  body.append(
    html`<div class="callout">
      ...
      αντίστοιχα) — όχι επειδή οι κάτοικοί τους ψήφισαν λιγότερο μαζικά, αλλά επειδή πολλοί παραμένουν εγγεγραμμένοι στον
